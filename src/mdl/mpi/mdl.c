...
void mdlGetTimer(MDL mdl, mdlTimer *t0, mdlTimer *t)
{
  ...
  */
  t->wallclock = MPI_Wtime() - t0->wallclock;
...
void mdlPrintTimer(MDL mdl,const char *message, mdlTimer *t0) 
{
  mdlTimer lt;
...
void mdlDiag(MDL mdl,const char *psz)
{
	if (mdl->bDiag) {	
...
void mdlReqService(MDL mdl,int id,int sid,void *vin,size_t nInBytes)
{
	char *pszIn = vin;
	/*
 ...
 */
CACHE *CacheInitialize(MDL mdl,int cid,void *pData,size_t iDataSize,size_t nData)
{
	CACHE *c;
	int i,nMaxCacheIds;
	int first;
 ...
 */
void mdlROcache(MDL mdl,int cid,void *pData,size_t iDataSize,size_t nData)
{
	CACHE *c;
	int id;
	CAHEAD caIn;
 ...
 */
void mdlCOcache(MDL mdl,int cid,void *pData,size_t iDataSize,size_t nData,
