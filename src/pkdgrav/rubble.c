/* suppress pedantic "empty translation unit" warning if !RUBBLE */
...
#define RUBBLE_NOFRIC 0 /* No dynamical friction or gas friction */
#define RUBBLE_DYNAMICAL 1 /* Just dynamical friction */
#define RUBBLE_GAS 2 /* Just gas friction */
#define RUBBLE_DYNANDGAS 3 /* Both gas and dynamical friction */

#define RUBBLE_FRICTION RUBBLE_NOFRIC	
#define EXPANSION_FACTOR 6.0 /* currently only used in the drag calcs! */
...
rubLocateBin(PARTICLE *p,DUST_BINS_PARAMS *DB,double M,double *a,int *piBin)
{
	...
	/* semimajor axis */
	//*a = 1.0/(2.0/r - v2/(M + p->fMass)); XXX
	*a = r;

	/*	if (p->iOrder == 1065) 
	    printf("r = %e, v2 = %e, M = %e, mp= %e, a = %e\n",
	    r,v2,M,p->fMass, *a);*/
/*	printf("particle %d has semimajor axis = %f\n",p->iOrder, *a);*/
	if (*a <= 0.0 && p->iColor != PLANETESIMAL) {/*DEBUG*/
	  ...
	  if (*a <= 0.0) {
	    printf("iOrder = %d, r = %e, v2 = %e, M = %e, mp= %e, a = %e\n",
		   p->iOrder,r,v2,M,p->fMass, *a);
	    printf("x = %e, y = %e, z = %e\n", p->r[0],p->r[1],p->r[2]);
						...
						double dt,double M,double pDustBinsMassLoss[])
{
	...
	for (i=0;i<nLocal;i++) {
		...
		if (p->iBin < 0 || p->iBin >= DB->nDustBins)
			...
			/* need to get around round-off error somehow... */
			if (fabs(x) < TINY)
			  e = TINY;
			else
			  e = sqrt(x);

			// XXX change to stop assertion error caused by e being nan
			if (isnan(e)) e = 0.0;
			//assert(e < 1.0);
...
/*DEBUG following function fails pedantic compilation checking due to
  setting unused variables, which occurs because RUBBLE_FRICTION
  should really be a variable rather than a macro...*/

/* ZML adding dynamical friction with inclination dispersion = 0, 22.05.07 */
...
)
{
...
#ifndef RUBBLE_FRICTION
	assert(0);
#endif
	if (bFirstCall) {
		(void) printf("RUBBLE FRICTION OPTION = %i\n",RUBBLE_FRICTION);
		(void) printf("EXPANSION FACTOR = %g\n",EXPANSION_FACTOR);
		}
	for (i=0;i<pkd->nLocal;i++) {
		...
		/* compute change to orbit, based on momentum conservation */
			...
			/*			omega = sqrt(M + p->fMass)/pow(SQ(p->r[0]) + SQ(p->r[1]),0.75);*/ 
			omega = sqrt(M)/pow(SQ(p->r[0]) + SQ(p->r[1]),0.75); /* ang speed of dust in plane */
		 ...
		 */
			...
			r = sqrt(SQ(p->r[0])+SQ(p->r[1])+SQ(p->r[2])); /* should be a? */
			v2 = SQ(p->v[0])+SQ(p->v[1])+SQ(p->v[2]);
...
#if (RUBBLE_FRICTION == RUBBLE_DYNAMICAL || RUBBLE_FRICTION == RUBBLE_DYNANDGAS)
			...
			if (sin_incl2 >= dVdisp) { /* assumes Max dist for background see BT p 424 */
/*				printf("sin_incl greater, sin_incl = %e, dVdisp = %e\n", sin_incl2, dVdisp);*/
...
/*				printf("sin_incl less, sin_incl = %e, incl = %e, hz = %e, h = %e\n", sin_incl2, incl, hz, h);*/
...
/*
			if (p->iOrder == 4188) {
			  printf("mass = %e, pos_x = %e, pos_y = %e, pos_z = %e, a = %e\n", 
				 p->fMass,p->r[0],p->r[1],p->r[2],a);
			  printf("vkx = %e, vky = %e,vx = %e, vy = %e, vz = %e, vdx = %e, vdy = %e, vdyz = %e\n",
				 vkx,vky,p->v[0],p->v[1],p->v[2],vdynx, vdyny, vdynz);
			  printf("Coulomb = %e\n", coulomb);	
			 }
*/
			/* velocity corrected for dynamical friction and accretion of dust */
...
#if (RUBBLE_FRICTION == RUBBLE_GAS || RUBBLE_FRICTION == RUBBLE_DYNANDGAS)

#define SIGMA_G (1700*1.495e13*1.495e13/1.9891e33) /* 1700 in g/cc converted to pkdgrav units */
#define C_S (sqrt(1.4*1.38e-23*400/(2*1.67e-27))/30000.0) /* gamma = 1.4 (for H_2), T = 400 K, m=2m_H converted to pkdgrav units */ 
			{
			double omega,e,tau_damp,k;
			omega = sqrt((M + p->fMass)/(a*a*a));
			e = h*h/(a*(M + p->fMass));
			if (e > 1.0 + 1.0e-6) { /*DEBUG*/
			...
			/* following assumes all velocity components damp equally, and that the eccentricity is small (so that e ~ (v-vk)/vk) */
...
/*				printf("particle %d,mass %e, color %d, rung %d is kept\n", 
				...
				assert(rho <= 2e4); /*DEBUG really need to adjust radii of rubble particles that never merged -- shouldn't be needed until late in a simulation when rubble particle masses exceed the minimum planetesimal mass*/
				}
				}
			else {
...
#ifdef ORIGIN_HISTOGRAM
					MergeHistograms(aDustBins[iBin].dMass, aDustBins[iBin].origin_bins, p->fMass, p->origin_bins);
#endif /* ORIGIN_HISTOGRAM */ 
					aDustBins[iBin].dMass += p->fMass;
				} else {
#ifdef ORIGIN_HISTOGRAM
					MergeHistograms(pDustBinsTrash->dMass, pDustBinsTrash->origin_bins, p->fMass, p->origin_bins);
					pDustBinsTrash->dMass += p->fMass;
#endif /* ORIGIN_HISTOGRAM */ 
...
void pkdRubInterpCleanup(PKD pkd,DUST_BINS_PARAMS *DB,double M,int iOrder,
						 int *iBin,DustBins *pDustBinsInterp)
{
	PARTICLE *p;
	double a;
	int i,nLocal = pkdLocal(pkd);
...
/*	printf("iOrder = %i, dDustBinsInterpMass = %e\n",iOrder,*dDustBinsInterpMass);
	...
	for (i=0;i<nLocal;i++) {
		...
		/*	printf("p->dDustMass = %e\n",p->dDustMass);*/
			pDustBinsInterp->dMass = p->dDustMass;
#ifdef ORIGIN_HISTOGRAM
			for (j=0;j<NUM_ORIGIN_BINS;j++)
				pDustBinsInterp->origin_bins[j] = p->origin_bins[j];
#endif /* ORIGIN_HISTOGRAM */
...
/* Initializing col acc - NOTE this is not used but is in the */
/* collider structure. To avoid FPEs using icc have set it to */
/* zero - calc has not been checked. 12.07.06 */

	ZERO_VEC((*col_c)->a);

/*	COPY_VEC(col_a->a,a_temp_a);
	SCALE_VEC(a_temp_a,col_a->fMass);
	COPY_VEC(col_b->a,a_temp_b);
	SCALE_VEC(a_temp_b,col_b->fMass);
	ADD_VEC(a_temp_a,a_temp_b,acom);
	COPY_VEC(acom,(*col_c)->a);
	NORM_VEC((*col_c)->a,col_a->fMass + col_b->fMass);*/
...
rubRanVel(double speed_max,VECTOR v)
{
	...
	do {
		SET_VEC(v,randUniform(),randUniform(),randUniform());
		SCALE_VEC(v,v_max);
				   ...
				   RUBBLE_PILE *rp_proj,int proj_idx,COLLIDER *col) 
{
	...
	/*
	 ** Note - the acceleration vector in the collider stucture is not
	 **	used in any rubble calculations, further more, the RUBBLE_PILE
	 **	stucture -> SSDATA structure has no acc. info.
	 */
...
/*		printf("Particle radius = %e, target radius = %e\n",
		...
		printf("rp_proj.n_particles=%i,rp_target.n_particles=%i\n",
...
#ifdef ORIGIN_HISTOGRAM
	{
	/* Copy target's origin_bin to new COLLIDERs, then do proj's. */
	/* NOTE: this assumes equal-mass rubble particles! */
	int i = 0, j = 0;
	for(i = 0; i < rp_target.n_particles; ++i) {
		for(j = 0; j < NUM_ORIGIN_BINS; ++j)
			(*col_c)[i].origin_bins[j] = target->origin_bins[j];
	}
	for(i = 0; i < rp_proj.n_particles; ++i) {
		for(j = 0; j < NUM_ORIGIN_BINS; ++j)
			(*col_c)[i + rp_target.n_particles].origin_bins[j] = proj->origin_bins[j];
	}
	}
#endif /* ORIGIN_HISTOGRAM */
...
void rubRubbleCollide(const COLLIDER *col_a,const COLLIDER *col_b,
					  ...
					  double *dMassInDust,int *nOut)
{
	...
	INTERP_PARAMS ip;
	double m,m_prim;
/* if f > 1 no need to define w2, w2max, and i */
/*	double w2,w2max;*/
/*	int i;*/
...
/* use the following when f=1 other wise comment out spin test */
...
#ifdef ORIGIN_HISTOGRAM
			/* origin histogram update handled in pkdRubInterpCleanup() */
#endif /* ORIGIN_HISTOGRAM */
			}
		/*
