...
/*DEBUG following functions should be collected somewhere else... (also see smoothfcn.c)*/
int sign(double A);
...
void vectorGetBasis(Vector a,Vector b,Vector c)
{
	...
	/* (see matrixOrthonormalize() for a similar procedure) */
...
void vectorRotate(Vector vPoint,const Vector vAxis,double dAngle)
{
	/*
	** Rotates vPoint around vAxis (a unit vector) by dAngle radians.
	...
	*/
	...
	sa = sin(dAngle);
	ca = cos(dAngle);
	vectorRotate2(vPoint,vAxis,sa,ca);
	}

void vectorRotate2(Vector vPoint,const Vector vAxis,double sa,double ca)
{
	/*
	** Rotates vPoint around vAxis (a unit vector) by the angle that
...
void vectorGetClosestPointOnLine(const Vector A,const Vector B,const Vector P,Vector Q)
{
	/*
	...
	*/
	...
	vectorSub(A,B,v1);
	vectorSub(P,B,v2);
	T = vectorDot(v1,v2)/vectorMagSq(v1);
	vectorScale(v1,T,v1);
	vectorAdd(B,v1,Q);
}

void vectorGetClosestPointOnSegment(const Vector A,const Vector B,const Vector P,Vector Q)
{
	/*
...
void matrixQuatRot(const Vector vAxis, double dAngle, Matrix R)
{
	/*
	** Given a rotation axis (unit vector) and rotation magnitude
	** (angle in radians), this routine returns a 3x3 rotation
	** matrix, constructed via a quaternion.
	*/

	double sa,a,b,c,d;

	/* the quaternion vector is q = (a, b, c, d) */
...
static void _proj(const Vector v, const Vector u, Vector p)
{
	double u2 = vectorDot(u, u);
	assert(u2 > 0);
	vectorScale(v, vectorDot(v, u) / u2, p);
}

void matrixOrthonormalize(Matrix a, int priority_row)
{
	/* Uses a Gram-Schmidt method to create an orthonormal basis
	from the rows of a given matrix. One vector is left untouched,
	which should be specified by the user by an integer argument
	corresponding to a row of the matrix that is passed in the
	first argument. Only works for 3-dimensional matrices (as with
	most other functions here). */

	Vector vTemp, vTemp1;
	double dDot;

	if (priority_row != 0)	//Ensure that the desired row is left untouched
	{
		matrixSwapRows(a, priority_row, 0);
		}

	/* Orthogonalize vectors by removing projections of other components in basis */
	_proj(a[1], a[0], vTemp);
	vectorSub(a[1], vTemp, a[1]);	//Remove component of a[0] from a[1] -> orthogonal
	_proj(a[2], a[0], vTemp);
	vectorSub(a[2], vTemp, vTemp1);	//Remove component of a[0] from a[2] -> orthogonal
	_proj(a[2], a[1], vTemp);
	vectorSub(vTemp1, vTemp, a[2]);	//Remove component of a[1] from a[2] -> orthogonal
#ifdef OLD
	dDot = vectorDot(a[0],a[1]);
	vectorScale(a[0],dDot,vTemp);
	vectorSub(a[1],vTemp,a[1]);	//Removes component of a[0] from a[1] -> orthogonal

	dDot = vectorDot(a[0],a[2]);
	vectorScale(a[0],dDot,vTemp);
	vectorSub(a[2],vTemp,vTemp);	//Remove component of a[0] from a[2] -> orthogonal
	dDot = vectorDot(a[1],a[2]);
	vectorScale(a[1],dDot,vTemp1);
	vectorSub(vTemp,vTemp1,a[2]);	//Remove component of a[1] from a[2] -> orthogonal
#endif /* OLD */
	/*Normalize the now-orthogonal basis*/
	vectorNorm(a[0]);
	vectorNorm(a[1]);
	vectorNorm(a[2]);

	if (priority_row != 0) //Swap again to preserve original form of matrix
	{
		matrixSwapRows(a, priority_row, 0);
		}
	}

#ifdef OBSOLETE /*DEBUG replaced by routines in matrix.c and matrix.h*/
