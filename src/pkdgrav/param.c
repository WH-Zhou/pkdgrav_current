...
int prmParseParam(PRM prm,char *pszFile)
{
 ...
 syntax_error:
	...
	fprintf(stderr,"Syntax error in %s(%d):\n%s",pszFile,iLine,achBuf);
	fclose(fpParam);
	return(0);
 cmd_error:
	q = achBuf;
	...
	fprintf(stderr,"Unrecognized command in %s(%d):%s\n",pszFile,iLine,achBuf);
	fclose(fpParam);
	return(0);
	}
...
int prmArgProc(PRM prm,int argc,char **argv)
{
	...
	for (i=1;i<argc;++i) {
		...
		case 1:
			...
			++i;
			if (i == argc) {
				fprintf(stderr,"Missing integer value after command line ");
			...
			if (ret != 1) {
				fprintf(stderr,"Expected integer after command line ");
			    fprintf(stderr,"argument:%s\n",argv[i-1]);
				prmArgUsage(prm);
				return(0);
				}
		...
		case 2:
			...
			++i;
			if (i == argc) {
				fprintf(stderr,"Missing double value after command line ");
			    fprintf(stderr,"argument:%s\n",argv[i-1]);
				prmArgUsage(prm);
				return(0);
				}
			...
			if (ret != 1) {
				fprintf(stderr,"Expected double after command line ");
			    fprintf(stderr,"argument:%s\n",argv[i-1]);
				prmArgUsage(prm);
				return(0);
				}
		...
		case 3:
			...
			++i;
			if (i == argc) {
				fprintf(stderr,"Missing string after command line ");
			    fprintf(stderr,"argument:%s\n",argv[i-1]);
				prmArgUsage(prm);
				return(0);
				}
			assert(pn->iSize > (int) strlen(argv[i]));
