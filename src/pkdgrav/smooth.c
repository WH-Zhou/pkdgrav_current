...
int smBallGather(SMX smx,FLOAT fBall2,FLOAT *ri)
{
	...
	(void) iDum; /* to suppress "set but not used" compilation warning */
	x = ri[0];
	y = ri[1];
	z = ri[2];
	if (smx->bPeriodic) {
	...
	while (1) {
        ...
        if (smx->bPeriodic) {
          #ifdef SLIDING_PATCH
          	INTERSECT_DEM_FIXED_BALL_SLIDING_PATCH(p[pj].r[0],p[pj].r[1],p[pj].r[2],lx,ly,lz,x,y,z,sx,sy,sz,iDum);
          #else
          	INTERSECT_DEM_FIXED_BALL(p[pj].r[0],p[pj].r[1],p[pj].r[2],lx,ly,lz,x,y,z,sx,sy,sz,iDum);
          #endif
          }
        #endif
				dx = sx - p[pj].r[0];
...
void smSmooth(SMX smx,SMF *smf)
{
	...
	(void) iDum; /* to suppress "set but not used" compilation warning */
	/*DEBUG not used idSelf = smx->pkd->idSelf;*/
	nSmooth = smx->nSmooth;
