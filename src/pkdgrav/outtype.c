...
FLOAT VecType(PKD pkd, PARTICLE *p,int iDim,int iType)
{
  ...
  FLOAT vTemp;
	...
	case OUT_ORIGIN_BINS:
		return p->origin_bins[iDim];
...
#ifdef DEM_DIAG
	case OUT_DEM_DIAG:
		strncat(achFile, "demdiag", 256);
		break;
#endif /* DEM_DIAG */
#ifdef ORIGIN_HISTOGRAM
	case OUT_ORIGIN_BINS:
		strncat(achFile, "origin_bins", 256);
		break;
#endif /* ORIGIN_HISTOGRAM */ 
	default:
            assert(1);
...
#ifdef DEM_DIAG
					/*
					** NOTE: although we output information for all
					** particles, a given particle only has overlap
					** info for particles that have higher iOrder
					** numbers than it (due to the optimization
					** strategy employed in DoDEM()).  The full data
					** can be reconstructed however, since Newton's
					** 3rd law requires the forces recorded here to be
					** equal and opposite between particles.
					*/
				case OUT_DEM_DIAG: {
					PARTICLE *p;
					int j;
					for (i = 0; i < pkd->nLocal; i++) {
						...
						for (j = 0; j < MAX_NUM_OVERLAPS_PER_PARTICLE_FOR_WALLS; j++)
							fprintf(fp, "%i %g %g %g %g %g %g ",
									p->walloverlaps[j].iOrder,
									p->walloverlaps[j].vFn[0],
									p->walloverlaps[j].vFn[1],
									p->walloverlaps[j].vFn[2],
									p->walloverlaps[j].vFt[0],
									p->walloverlaps[j].vFt[1],
									p->walloverlaps[j].vFt[2]);
#endif /* WALLS */
						fprintf(fp, "\n");
						}
					break;
				...
				case OUT_ORIGIN_BINS:
					for (i = 0; i < pkd->nLocal; ++i) {
						...
						for (j = 0; j < NUM_ORIGIN_BINS; ++j)
							fprintf(fp, "%g ", pkd->pStore[i].origin_bins[j]);
						fprintf(fp, "\n");
						}
					break;
#endif /* ORIGIN_HISTOGRAM */ 
