#ifndef LINALG_HINCLUDED
#define LINALG_HINCLUDED
...
typedef Vector Matrix[3]; /*DEBUG can't declare "Matrix" with this def'n -- why? */
...
FLOAT vectorMagSq(const Vector v); /* vectorDot(v,v) */
FLOAT vectorMag(const Vector v); /* sqrt(vectorMag(v)) */
void vectorNorm(Vector v); /* vectorScale(v,1/vectorMag(v),v) */
...
void vectorSet(Vector v,double x,double y,double z); /* v[0]=x; v[1]=y; v[2]=z */
...
/*
** Given vector "a", returns orthonormal basis (a,b,c).
*/
void vectorGetBasis(Vector a,Vector b,Vector c);

/*
** Rotates vPoint around unit-vector vAxis by dAngle radians.  vPoint
** overwritten.
*/
void vectorRotate(Vector vPoint,const Vector vAxis,double dAngle);

/*
** Rotates vPoint around unit-vector vAxis by an angle with sine argument
** sa and cosine argument ca.  vPoint overwritten.
*/
void vectorRotate2(Vector vPoint,const Vector vAxis,double sa,double ca);

/*
** Given points A & B on a line, and point P, this routine returns the
** point Q on the line closest to P.
*/
void vectorGetClosestPointOnLine(const Vector A,const Vector B,const Vector P,Vector Q);

/*
** Given segment (finite line) AB and the point P, this routine
** returns the point Q on the segment closest to P.
*/
 ...
 *
 *  Parameters (in):
 *   a - The matrix in question (unmodified)
 *
 *  Parameters (out):
 *   b - Its transpose
 */
...
/** Construct a rotation matrix using a quaternion.
 *
 *  Parameters (in):
 *   vAxis - rotation axis (unit vector)
 *   dAngle - rotation angle (radians)
 *
 *  Parameter (out):
 *   R - rotation matrix (apply with matrixTransform() or matrixMultiply())
 */
void matrixQuatRot(const Vector vAxis, double dTheta, Matrix R);

/** Create an orthonormal basis from the rows of a matrix, using the
 *   Gram-Schmidt orthonormalization process.
 *
 *  Parameters (in/out):
 *   a - The source matrix, which is modified to contain the resulting matrix
 *
 *  Parameters (in):
 *   priority_row - The row whose direction will not be changed
 *
 */
