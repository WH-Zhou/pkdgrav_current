...
#ifdef AGGS
			/*
			** Skip self-interaction between particles in the same aggregate.
			** Assumes small aggs so all particles *likely* in this interaction list.
			** Does not check for negative iOrgIdx.
			*/
			if (p[i].iOrgIdx == p[j].iOrgIdx)
				continue;
#endif /* AGGS */
			dx = p[j].r[0] - p[i].r[0];
			dy = p[j].r[1] - p[i].r[1];
