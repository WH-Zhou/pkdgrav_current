...
/*
 ** Splitting these out instead of using "||" to make it cleaner to
 ** remove specific macros for export purposes...
 */

#if defined(GASOLINE)
#define NEED_VPRED
#elif defined(ROT_FRAME)
#define NEED_VPRED
#elif defined(SIMPLE_GAS_DRAG)
#define NEED_VPRED
#elif defined(SPRINGS)
#define NEED_VPRED
#elif defined(DEM)
#define NEED_VPRED
#endif

/* following could be user parameter (note bVWarnings still applies) */
...
/*	FLOAT fDensSave;*/	/* Used by diagnostic DensCheck funcs */
    ...
    int iActualAgg;     /* for saving true agg index during spinup */
#endif                  
  ...
  Matrix orient; /* principal axes as column vectors */
  ...
  double dXForceOnWalls[MAX_NUM_WALL_ASSEMBLIES];
  double dYForceOnWalls[MAX_NUM_WALL_ASSEMBLIES];
  double dZForceOnWalls[MAX_NUM_WALL_ASSEMBLIES];
  double dXTorqueOnWalls[MAX_NUM_WALL_ASSEMBLIES];
  double dYTorqueOnWalls[MAX_NUM_WALL_ASSEMBLIES];
  double dZTorqueOnWalls[MAX_NUM_WALL_ASSEMBLIES];
#endif /* DEM_WALLS_REACT */
#endif /* WALLS */
  DEM_STATS_PARTICLE DEMStats;
#ifdef DEM_FIXED_BALL
  double dNextLargestRad;
  ...
  */
  FLOAT r_body[3];
  FLOAT r_unwrapped[3];		//DeMartini
  FLOAT agg_COM[3]; //JCM 
  FLOAT DEM_aggaccels[3]; //JCM
  FLOAT DEM_aggtorques[3]; //JCM 
#endif /* AGGS */
...
#ifdef AGGS
#ifdef FAST_AGGS
  int iNextFastAgg;	/* The next particle in processor order for FAST_AGGS cache line */
  int iLastFastAggStart;/* First particle of the agg last acted on in FAST_AGGS cache line */
  int iLastFastAggIdx;	/* The agg index of the agg last acted on in FAST_AGGS cache line */
#endif
#endif
#ifdef COLLISIONS
