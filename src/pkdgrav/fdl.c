...
int FDL_error(FDL_CTX *CTX,int err,const char *file,int line)
{
...
void FDL_addcom(FDL_CTX *CTX,const char *com,int comnum)
{
    int l;
...
unsigned int hash_func(void *e)
{
    const unsigned char *p;
    unsigned int i;
    
    p = (const unsigned char *) ((ENTRY *)e)->ident;
    i = SCATTER+MAGIC;
    while (*p) i += (*(p++))*SCATTER;
    return(i);
    }
...
int isoneof(const char *s,char c)
{
    while (*s) {
        if (*s == '@' && c == 0) return(1);
        if (*s == c) return(1);
...
int FDL_expr(FDL_CTX *CTX,FDL_NODE **E,const char *retsyms)
{
    int command,tok,err;
...
int FDL_read(FDL_CTX *CTX,const char *ident,void *value)
{
    ENTRY e,*el;
    FDL_NODE *p;
 ...
 */
int FDL_write(FDL_CTX *CTX,const char *ident,void *value)
{
    ENTRY e,*el;
    FDL_NODE *p;
...
int FDL_index(FDL_CTX *CTX,const char *ident,int index)
{
    FDL_NODE *p,*old;
    ENTRY e,*el,e1,*el1;
    int max;
...
int FDL_offset(FDL_CTX *CTX,const char *ident)
{
    ENTRY e,*el;
    FDL_NODE *p;
