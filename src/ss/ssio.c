 ...
 **  6/30/17: added support for particle orientations (soft-sphere only).
 */
...
int ssioSetPos(SSIO *ssio, const size_t pos)
{
	assert (ssio != NULL);
    assert (pos < UINT_MAX);
	if (!xdr_setpos(&ssio->xdrs,(unsigned int) pos)) return 1;
    return 0;
}
#endif /* SSIO_USE_MPI */
...
void ssioStandardToReduced(const SSDATA *ds,SSRDATA *dr)
{
	...
	dr->fMass = (float) ds->mass;
	dr->fRadius = (float) ds->radius;
	for (i=0;i<N_DIM;i++)
		dr->vPos[i] = (float) ds->pos[i];
	dr->iColor = ds->color;
...
int ssioOriHead(SSIO *ssio,ORIHEAD *head)
{
	assert(ssio != NULL && head != NULL);
	...
	if (head->iMagicNumber != SSIO_MAGIC_STANDARD &&
		head->iMagicNumber != SSIO_MAGIC_REDUCED) {
		fprintf(stderr,"Invalid SSIO magic number (%i).\n",head->iMagicNumber);
		return 1;
		}
...
int ssioOriData(SSIO *ssio,ORIDATA *data)
{
	int i;

	assert(ssio != NULL && data != NULL);
	for (i=0;i<N_DIM;i++)
		if (!xdr_double(&ssio->xdrs,&data->p1[i])) return 1;
	for (i=0;i<N_DIM;i++)
		if (!xdr_double(&ssio->xdrs,&data->p2[i])) return 1;
	return 0;
	}

int ssioOriDataReduced(SSIO *ssio,ORIRDATA *data)
{
	int i;

	assert(ssio != NULL && data != NULL);
	for (i=0;i<N_DIM;i++)
		if (!xdr_float(&ssio->xdrs,&data->p1[i])) return 1;
	for (i=0;i<N_DIM;i++)
		if (!xdr_float(&ssio->xdrs,&data->p2[i])) return 1;
	return 0;
	}
