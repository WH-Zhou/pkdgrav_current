#define SS_CORE /* needed for compatability with this package */
/*
 ** companion.c --- Version 1.0, 07.27.04 (ZML and DCR)
 ** searches for binaries 
 **
 ** Version 1.1 6/18/11 by DCR (updated SSIO interface).
 */
...
static void calc_bin_stats(const SSDATA *p1,const SSDATA *p2,int flag,BIN_STATS *bs)
{
	VECTOR r,v;
	double M_inv;
...
static int sort_bin(const void *a,const void *b)
{
...
static double hill(const PARAMS *p,double mass_i,double mass_c)
{
...
static int myNewExt(const char *infile,const char *inext,
...
static void add_to_list(COMPDATA *prim,COMPDATA *sat,BINARY **list,long *list_size,long *list_posn) 
{
	...
	if (*list_posn >= *list_size) { 
		...
		*list = (BINARY *) realloc((void *) (*list),(size_t) (*list_size)*sizeof(BINARY));
		assert(*list != NULL);
...
static void find_companion(const PARAMS *p,const NODE *node,COMPDATA *part,
...
static void get_com_vel(NODE *node)
{
...
static void add_to_tree(NODE *node,COMPDATA *p)
{
...
static int read_data(const PARAMS *p,const char *file_in,COMPDATA **part,
					 long *n,double *m_tot,VECTOR root_center,double *root_size)
{
	/* read data from ss file */

	SSDATA *d;
	FILE *fp = NULL; /* for txt & bin file types only */
	...
	case FileTypeSS:
		...
		if (h.iMagicNumber != SSIO_MAGIC_STANDARD) {
			(void) fprintf(stderr,"Only standard ss format supported\n");
			(void) ssioClose(&ssio);
			return 1;
			}
		if (h.n_data <= 0) {
	...
	*part = (COMPDATA *) malloc((size_t) (*n)*sizeof(COMPDATA)); /*allocate space for part*/
	...
	case FileTypeBin:
		...
		while (feof(fp) == 0) {
			if (i>=(*n)) {
				...
				*n *= BUF_SIZE_MULT;
				*part = (COMPDATA *) realloc((void *) (*part),(size_t) (*n)*sizeof(COMPDATA));
				assert(*part != NULL);
	...
	*part = (COMPDATA *) realloc((void *) (*part),(size_t) (*n)*EXTRA_STORE*sizeof(COMPDATA));
...
static void make_com_part(BINARY *tightest,long *n_part,long *part_buf_size,
						  COMPDATA **part)
{
	/* 
	...
	if (*n_part == *part_buf_size) {
		...
		*part = (COMPDATA *) realloc((void *) (*part),(size_t) (*part_buf_size)*sizeof(COMPDATA));
		assert(*part != NULL);
...
static BOOLEAN ok_to_cut(const PARAMS *p,const COMPDATA *prim,const COMPDATA *sat,const BIN_STATS *bs)
{
...
static void cut_list(const PARAMS *p,BINARY *list,long *list_length)
{
	...
	flag = (BOOLEAN *) malloc((size_t) (*list_length)*sizeof(BOOLEAN));
...
static int extract(const PARAMS *p,const char *filename,const BINARY *list,
				   long nbin)
{
	...
	if (p->FileType == FileTypeSS) {
		head.time = 0.0;
		head.n_data = (int) (n + 1); /* one primary plus n satellites */
...
#ifdef SS_CORE
static int find_real_part(const PARAMS *p,FILE *fp,SSIO *ssio_out,COMPDATA *part,long *n)
#else
static int find_real_part(const PARAMS *p,FILE *fp,COMPDATA *part,long *n)
#endif
{
...
static int hier_extract(const PARAMS *p,const char *filename,COMPDATA *part)
{
...
static int write_tipsy(const char *filename,long npart,const BINARY *list,
					   long nbin)
{
...
static void set_defaults(PARAMS *params) 
{	
	/* parameters defaults */
...
static void parse_in(int argc,char *argv[],PARAMS *p)
{
...
static int write_output(const PARAMS *p,const char *filename_in,const BINARY *list,long nbin,double m_tot)
{
	BIN_STATS bs;
	const SSDATA *pd,*sd;
	FILE *fp_pr,*fp_ana;
...
static int walk_system(const PARAMS *p,FILE *fp,const COMPDATA *part,double m_tot,BOOLEAN write_header,int *n_layer)
{
	BIN_STATS bs;
	const SSDATA *pd,*sd;
...
static void create_summary(const PARAMS *p,const COMPDATA *part,HIER_OUTPUT *sum,double *max_a)
{
	BIN_STATS bs;
	const SSDATA *pd,*sd;
...
static int sort_mass(const void *a, const void *b)
{
...
static int write_hier_output(const PARAMS *p,const char *filename,const COMPDATA *part,long n_part,double m_tot,long n_orig)
{
	BIN_STATS bs;
	const SSDATA *pd,*sd;
	HIER_OUTPUT *sum;
	...
	sum = (HIER_OUTPUT *) malloc((size_t) sum_buf_size*sizeof(HIER_OUTPUT));
	assert(sum != NULL);
	...
	for (i=0;i<n_part;i++) {
		...
		if (part[i].com == NULL && part[i].prim != NULL) { /* top of system tree */
			...
			if (++n_sys == sum_buf_size) {
				...
				sum = (HIER_OUTPUT *) realloc((void *) sum,(size_t) sum_buf_size*sizeof(HIER_OUTPUT));
				assert(sum != NULL);
				} 
			}
		}

	qsort((void *) sum,(size_t) n_sys,sizeof(HIER_OUTPUT),sort_mass); /* sort systems with most massive first */
	n_true_hier = 0;
	for (i=0;i<n_sys;i++) {
...
static int sort_per(const void *a,const void *b)
{
	const BINARY *b1,*b2;
	const SSDATA *p1,*p2;
...
static void calc_per(BINARY *list,long n)
{
	BIN_STATS bs;
	int i;
...
static void find_systems(const PARAMS *p,COMPDATA **part,long *npart,
						 ...
						 BINARY **bin,long *bin_buf_size,long *nbin)
{
	...
	calc_per(*bin,*nbin);
	qsort((void *) *bin,(size_t) *nbin,sizeof(BINARY),sort_per);
	...
	del = (long *) malloc((size_t) del_buf_size*sizeof(long));
	assert(del != NULL);
	add_buf_size = BUF_SIZE_INIT;
	add = (BINARY *) malloc((size_t) add_buf_size*sizeof(BINARY));
	assert(add != NULL);
	new_buf_size = *bin_buf_size;
	new = (BINARY *) malloc((size_t) new_buf_size*sizeof(BINARY));
	assert(new != NULL);
	...
	while (*nbin > 0) {
		...
		if ((double) ncmp / (double) ntree < TREE_REBUILD_FRAC || 
		...
		for (ndel=ibin=0;ibin<*nbin;ibin++) {
			...
			if (IS_COM_MBR(binary->prim) || IS_COM_MBR(binary->sat)) {
				...
				if (++ndel == del_buf_size) {
					...
					del = (long *) realloc((void *) del,(size_t) del_buf_size*sizeof(long));
					assert(del != NULL);
					}
				}
			}
		...
		calc_per(add,nadd);
		qsort((void *) add,(size_t) nadd,sizeof(BINARY),sort_per);
		...
		while (ibin < *nbin || iadd < nadd) {
			...
			while (iadd < nadd &&
				...
				if (++nnew == new_buf_size) {
					...
					new = (BINARY *) realloc((void *) new,(size_t) new_buf_size*sizeof(BINARY));
					assert(new != NULL);
					}
				}
...
int main(int argc,char *argv[])
{
	...
	for(;optind<argc;optind++) {
		...
		if (params.Hier == TRUE) {
			...
			(void) printf("Sorting...\n");
			qsort((void *) list,(size_t) list_posn,sizeof(BINARY),sort_bin);
