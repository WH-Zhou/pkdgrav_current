 ...
 **  6/30/17: added particle orientation support (soft-sphere only).
 */
...
/* particle orientation defines */

#define ORIHEAD SSHEAD

#define ORIHEAD_SIZE SSHEAD_SIZE

typedef struct ori_data {
  double p1[N_DIM],p2[N_DIM];
  } ORIDATA;

#define ORIDATA_SIZE (2*N_DIM*sizeof(double))

typedef struct orir_data {
  float p1[N_DIM],p2[N_DIM];
  } ORIRDATA;

#define ORIRDATA_SIZE (2*N_DIM*sizeof(float))
